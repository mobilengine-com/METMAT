...
let avgRunTime = db.environmentValues.Read({name:"hrAvgTestRuntime"})[0].value

Log(left_from_today_total)
...
function calculateFinish(req_days:number, holidays:number, currentDate:dtl, acumulated_holidays:number) {
...
if (form.recRun.submitter) {
    ...
    testerForTesting.forEach(a => sum_participate += a.participation);
    ...
    requiredWorkingHours = float.Parse(requiredWorkingHours.toFixed(2))
...
if (form.recWrite.submitter) {
    ...
    let sum_participate = 0
    dbTesterForTesting.forEach(a => sum_participate += a.participation);
    Log("sum_participate: "+sum_participate)
    let totalTestToWriteCount = 0
    db.test_to_be_write_category.ReadFields({active: 1},["planned_count"]).forEach(a=> totalTestToWriteCount += a.planned_count)
    ...
    let requiredWorkingHoursToWrite = float.Parse((((totalTestToWriteCount - totalTestWriteDone) * avgRunTime)/sum_participate).toFixed(2))
